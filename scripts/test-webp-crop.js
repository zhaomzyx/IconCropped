...
async function testWebPCrop() {
  try {
    ...
    // 计算自适应阈值
    function calculateAdaptiveThreshold(data, width, height, channels, backgroundColor) {
      const colorDiffs = [];
      ...
      const adaptiveThreshold = Math.max(30, p25 * 0.5);
      console.log(`Panel threshold: ${adaptiveThreshold} (adaptive)`);

      return adaptiveThreshold;
    }

    const threshold = calculateAdaptiveThreshold(data, scaledWidth, scaledHeight, info.channels, backgroundColor);
