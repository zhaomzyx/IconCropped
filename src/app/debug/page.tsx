...
export default function WikiDebugPage() {
  ...
  const [cropResults, setCropResults] = useState<unknown[]>([]); // 裁切结果
      ...
      try {
        ...
        if (saved) {
          ...
          if (parsed.emptyIconVarianceThreshold === undefined) {
            mergedParams.emptyIconVarianceThreshold =
              DEFAULT_PARAMS.emptyIconVarianceThreshold;
          }
  ...
  const scanVerticalLine = useCallback(
    (
      ...
      console.log(
        `  colorTolerance=${colorTolerance}, sustainedPixels=${sustainedPixels}`,
      );
      ...
      if (scanLineX < 0 || scanLineX >= width) {
        console.warn(
          `[Y轴检测] ❌ Scan line X (${scanLineX}) is out of image bounds (${width})`,
        );
        return panels;
      }
      ...
      if (scanStartY < 0 || scanStartY >= height) {
        console.warn(
          `[Y轴检测] ❌ Scan start Y (${scanStartY}) is out of image bounds (${height})`,
        );
        return panels;
      }
      ...
      console.log(
        `[Y轴检测] 参数: scanLineX=${scanLineX}, scanStartY=${scanStartY}, colorTolerance=${colorTolerance}, sustainedPixels=${sustainedPixels}`,
      );
  ...
  const scanHorizontalLine = useCallback(
    (
      ...
      console.log(
        `[X轴检测] 扫描线 Y: ${scanY}, 背景色: (${backgroundColor.join(", ")})`,
      );
      console.log(
        `[X轴检测] 参数: colorTolerance=${colorTolerance}, sustainedPixels=${sustainedPixels}`,
      );
        ...
        if (diff > colorTolerance) {
          ...
          if (!inPanel && consecutivePanel >= sustainedPixels) {
            ...
            if (icons.length === 0) {
              ...
              console.log(
                `[X轴检测] Icon ${icons.length + 1} 左边界: ${iconStartX}`,
              );
            }
          }
        } else {
          ...
          if (inPanel && consecutiveBg >= sustainedPixels) {
            ...
            console.log(
              `[X轴检测] Icon ${icons.length} 右边界: ${iconEndX}, 中心点: ${iconCenterX.toFixed(1)}`,
            );
          }
        }
      }
      ...
      if (inPanel) {
        ...
        console.log(
          `[X轴检测] 大panel右边界: ${panelEndX} (到图片边界), 最后一个Icon中心点: ${iconCenterX.toFixed(1)}`,
        );
      } else {
        // 使用最后一个icon的右边界
        if (icons.length > 0) {
          ...
          console.log(
            `[X轴检测] 大panel右边界: ${panelEndX} (基于最后一个icon计算)`,
          );
        }
      }
      ...
      console.log(
        `[X轴检测] 检测到 ${icons.length} 个icon, 宽度: ${panelEndX - panelStartX}`,
      );
  ...
  const drawCanvas = useCallback(() => {
    ...
    img.onload = () => {
      ...
      console.log(
        `Canvas尺寸：实际=${img.width}x${img.height}, 显示=${canvas.clientWidth}x${canvas.clientHeight}`,
      );
      console.log(
        `缩放比例：X=${canvas.clientWidth / canvas.width}, Y=${canvas.clientHeight / canvas.height}`,
      );
      ...
      const panelRanges = panelVerticalRanges.map((vRange, index) => {
        ...
        if (hRange) {
          console.log(
            `[Panel ${index + 1}] 检测到 Panel 边界: startX=${hRange.startX}, endX=${hRange.endX}, width=${hRange.endX - hRange.startX}`,
          );
        } else {
      ...
      for (
        let i = 0;
        i < Math.min(debugPanels.length, panelRanges.length);
        i++
      ) {
        ...
        if (isSelected) {
          ...
          console.log(`[Y轴检测结果]`);
          console.log(
            `  startY = ${range.startY}, endY = ${range.endY}, height = ${range.height}`,
          );
          console.log(`[X轴检测结果]`);
          console.log(
            `  startX = ${range.startX}, endX = ${range.endX}, width = ${range.width}`,
          );
        }
        ...
        console.log(
          `[扫描范围] 原始: (${range.startX}, ${range.startY}) ${range.width}x${range.height}`,
        );
        console.log(
          `[扫描范围] 缩小: (${scanX}, ${scanY}) ${scanWidth}x${scanHeight}`,
        );
        ...
        console.log(
          `  检测到 ${bounds.rows.length} 行, ${bounds.cols.length} 列`,
        );
        ...
        if (bounds.rows.length > 0) {
          ...
          console.log(
            `  ✅ 列检测使用第一行高度: ${Math.round(colScanHeight)}px`,
          );
        } else {
          console.log(
            `  ⚠️ 列检测使用整个panel高度: ${range.height}px (未检测到行)`,
          );
        }
        ...
        if (bounds.rows.length === 0) {
          ...
          console.warn(
            `    - 增大窗口高度 (当前: ${params.boundsWindowHeight})`,
          );
          console.warn(
            `    - 降低行检测方差阈值 (当前: ${params.boundsVarianceThresholdRow})`,
          );
          console.warn(
            `    - 降低最小行高 (当前: ${params.boundsMinRowHeight})`,
          );
        }
        ...
        if (bounds.cols.length === 0) {
          ...
          console.warn(
            `    - 增大窗口宽度 (当前: ${params.boundsWindowWidth})`,
          );
          console.warn(
            `    - 降低列检测方差阈值 (当前: ${params.boundsVarianceThresholdCol})`,
          );
          console.warn(
            `    - 降低最小列宽 (当前: ${params.boundsMinColWidth})`,
          );
        }
        ...
        if (bounds.rows.length > 0) {
          bounds.rows.forEach((row, i) => {
            console.log(
              `  行 ${i}: y=${row.topY} ~ ${row.bottomY}, 高度=${row.height}`,
            );
          });
        }
        ...
        if (bounds.cols.length > 0) {
          bounds.cols.forEach((col, i) => {
            console.log(
              `  列 ${i}: x=${col.leftX} ~ ${col.rightX}, 宽度=${col.width}`,
            );
          });
        }
        ...
        // 绘制行列边界线（始终显示，选中的更明显）
        const rowStrokeColor = isSelected
          ? "#22C55E"
          : "rgba(34, 197, 94, 0.4)";
        const rowLineWidth = isSelected ? 2 : 1;
        const colStrokeColor = isSelected
          ? "#EF4444"
          : "rgba(239, 68, 68, 0.4)";
        ...
        bounds.rows.forEach((row) => {
          ...
          if (isSelected) {
            ...
            ctx.font = "10px Arial";
            ctx.fillText(
              `行${row.rowIndex}: y=${Math.round(row.topY)}~${Math.round(row.bottomY)}`,
              scanX + 5,
              row.topY - 3,
            );
          }
        ...
        // 先绘制列检测的扫描范围（浅色矩形）
        const colScanHeight =
          bounds.rows.length > 0 ? bounds.rows[0].height : scanHeight;
        if (isSelected) {
        ...
        bounds.cols.forEach((col) => {
          ...
          if (isSelected) {
            ...
            ctx.font = "10px Arial";
            ctx.fillText(
              `列${col.colIndex}: x=${Math.round(col.leftX)}~${Math.round(col.rightX)}`,
              col.leftX + 3,
              scanY + 12,
            );
          }
        ...
        const validIcons = boundsIcons.filter((icon) => {
          ...
          if (isEmpty) {
            console.log(
              `  [过滤空图标] [${icon.row},${icon.col}] 方差=${variance.toFixed(2)} < 阈值=${params.emptyIconVarianceThreshold}`,
            );
          }
        ...
        console.log(
          `[空图标过滤] 原始图标数=${boundsIcons.length}, 有效图标数=${validIcons.length}, 过滤掉=${boundsIcons.length - validIcons.length}个`,
        );
        console.log(
          `[合成链信息] 该合成链包含 ${validIcons.length} 个合成物（小panel）`,
        );
        console.log(
          `  🌟 合成物数量: ${validIcons.length} 个（过滤掉 ${boundsIcons.length - validIcons.length} 个空方框）`,
        );
        console.log(`  📐 方差计算：基于原始边界（不应用1:1强制）`);
        console.log(`  📐 1:1强制：只在绘制和导出时应用`);
        if (params.forceSquareIcons) {
          console.log(
            `  📐 偏移校准：X=${params.forceSquareOffsetX}px, Y=${params.forceSquareOffsetY}px`,
          );
        }
        ...
        if (isSelected) {
          ...
          ctx.fillText(
            `预计图标: ${bounds.rows.length * bounds.cols.length}个`,
            scanX + scanWidth - 155,
            scanY + 32,
          );
          ctx.fillText(
            `合成物数量: ${validIcons.length}个`,
            scanX + scanWidth - 155,
            scanY + 49,
          );
          ctx.fillText(
            `列扫描高度: ${Math.round(colScanHeight)}px`,
            scanX + scanWidth - 155,
            scanY + 66,
          );
        }
        ...
        validIcons.forEach((icon, iconIndex) => {
          const { leftX, topY, width, height, centerX, centerY, row, col } =
            icon;

          console.log(
            `  合成物 #${iconIndex}: 行${row}列${col}, 位置(${Math.round(centerX)}, ${Math.round(centerY)})`,
          );
        ...
        if (isSelected) {
          ...
          const backgroundColor = getPixelColor(
            params.scanLineX,
            params.scanStartY,
          );
          ...
          ctx.fillText(
            `X=${params.scanLineX}, T=${params.colorTolerance}, S=${params.sustainedPixels}`,
            params.scanLineX + 5,
            params.scanStartY - 10,
          );
        }
      }
      ...
      console.log(
        `[drawCanvas] 已保存 ${currentDetectedPanels.length} 个面板的框体坐标`,
      );
        ...
        for (const line of lines) {
          ...
          if (trimmedLine.startsWith("event:")) {
            ...
            try {
              ...
              // 🌟 修改：解析错误也不阻断流程
              logInfo(
                `⚠️ 解析事件失败: ${e instanceof Error ? e.message : "未知错误"}`,
              );
            }
  ...
  const handleParamChange = (
    ...
    try {
      ...
      if (saved) {
        ...
        if (savedValue !== value) {
          console.error(
            `❌ [LocalStorage保存失败] 期望: ${value}, 实际: ${savedValue}`,
          );
        }
      }
    } catch (e) {
        ...
        if (parsed.params && typeof parsed.params === "object") {
          ...
          Object.keys(DEFAULT_PARAMS).forEach((key) => {
            if (key in parsed.params) {
              validParams[key as keyof typeof DEFAULT_PARAMS] =
                parsed.params[key];
            }
          });
  ...
  const handleExportToWorkbench = async () => {
    ...
    try {
      ...
      const exportPanels = detectedPanels.map((detectedPanel, i) => {
        ...
        logInfo(
          `  width=${detectedPanel.blueBox.width}, height=${detectedPanel.blueBox.height}`,
        );
        logInfo(`[绿框坐标]`);
        logInfo(
          `  x=${detectedPanel.greenBox.x}, y=${detectedPanel.greenBox.y}`,
        );
        logInfo(
          `  width=${detectedPanel.greenBox.width}, height=${detectedPanel.greenBox.height}`,
        );
        logInfo(`[红框坐标]`);
        ...
        logInfo(`  最终坐标:`);
        logInfo(
          `    BlueBox: x=${Math.round(detectedPanel.blueBox.x)}, y=${Math.round(detectedPanel.blueBox.y)}, w=${Math.round(detectedPanel.blueBox.width)}, h=${Math.round(detectedPanel.blueBox.height)}`,
        );
        logInfo(
          `    GreenBox: x=${Math.round(detectedPanel.greenBox.x)}, y=${Math.round(detectedPanel.greenBox.y)}, w=${Math.round(detectedPanel.greenBox.width)}, h=${Math.round(detectedPanel.greenBox.height)}`,
        );
    ...
    <div className="container mx-auto p-6">
      ...
      {imageUrl && (
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-200px)]">
          ...
          <div className="flex-1 overflow-y-auto pr-2">
            <Card>
              ...
              <CardContent>
                <canvas
                  ref={canvasRef}
                  className="max-w-full w-auto h-auto border border-gray-300"
                />
              </CardContent>
            </Card>
          </div>
          ...
          {/* 参数控制区域 - 固定在右侧，不随图片滚动 */}
          <div className="w-full lg:w-96 shrink-0 sticky top-0 h-full overflow-y-auto">
            <Card className="mb-4">
              <CardHeader>
                ...
                <div className="space-y-2">
                  {debugPanels.map((panel, idx) => {
                    const compositeCount =
                      detectedPanels[idx]?.redBoxes?.length || 0;
                    return (
                      <Button
                        key={idx}
                        variant={
                          selectedPanelIndex === idx ? "default" : "outline"
                        }
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                  ...
                  <div className="text-sm text-gray-700 space-y-2">
                    <p>
                      ...
                      {selectedPanelIndex < detectedPanels.length ? (
                        <>
                          {Math.round(
                            detectedPanels[selectedPanelIndex]?.blueBox.x || 0,
                          )}{" "}
                          ~{" "}
                          {Math.round(
                            (detectedPanels[selectedPanelIndex]?.blueBox.x ||
                              0) +
                              (detectedPanels[selectedPanelIndex]?.blueBox
                                .width || 0),
                          )}
                        </>
                      ) : (
                    ...
                    <p>
                      ...
                      {selectedPanelIndex < detectedPanels.length ? (
                        <>
                          {Math.round(
                            detectedPanels[selectedPanelIndex]?.blueBox.y || 0,
                          )}{" "}
                          ~{" "}
                          {Math.round(
                            (detectedPanels[selectedPanelIndex]?.blueBox.y ||
                              0) +
                              (detectedPanels[selectedPanelIndex]?.blueBox
                                .height || 0),
                          )}
                    ...
                    <p>
                      ...
                      {selectedPanelIndex < detectedPanels.length ? (
                        <>
                          {Math.round(
                            detectedPanels[selectedPanelIndex]?.blueBox.width ||
                              0,
                          )}{" "}
                          ×{" "}
                          {Math.round(
                    ...
                    <p>
                      ...
                      {selectedPanelIndex < detectedPanels.length ? (
                        <span className="text-purple-600 font-bold">
                          {detectedPanels[selectedPanelIndex]?.redBoxes
                            ?.length || 0}{" "}
                          个
                        </span>
                      ) : (
                ...
                <div className="border-l-4 border-green-500 pl-4">
                  <Label className="text-sm font-semibold text-green-600 mb-3 block">
                    绿框相关 (Title)
                  </Label>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            handleParamChange("gridStartY", v)
                          }
                          min={-200}
                          max={2000}
                          step={1}
                ...
                <div className="border-l-4 border-orange-500 pl-4">
                  <Label className="text-sm font-semibold text-orange-600 mb-3 block">
                    扫描线相关 (Scan)
                  </Label>
                  <div className="space-y-4">
                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        扫描线 X 坐标 (Scan Line X)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.scanLineX]}
                          onValueChange={([v]) =>
                            handleParamChange("scanLineX", v)
                          }
                          min={0}
                          max={100}
                          step={1}
                    ...
                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        扫描起始 Y 坐标 (Scan Start Y)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.scanStartY]}
                          onValueChange={([v]) =>
                            handleParamChange("scanStartY", v)
                          }
                          min={0}
                          max={500}
                          step={1}
                    ...
                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        颜色容差值 (Color Tolerance)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.colorTolerance]}
                          onValueChange={([v]) =>
                            handleParamChange("colorTolerance", v)
                          }
                          min={5}
                          max={50}
                          step={1}
                    ...
                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        连续判定高度 (Sustained Pixels)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.sustainedPixels]}
                          onValueChange={([v]) =>
                            handleParamChange("sustainedPixels", v)
                          }
                          min={5}
                          max={100}
                          step={5}
                    ...
                    <div className="border-t pt-4 mt-4">
                      <Label className="text-xs font-semibold text-orange-600 mb-3 block">
                        X轴检测参数 (Horizontal Scan)
                      </Label>
                      <div className="space-y-4">
                        <div>
                          <Label className="text-xs font-medium text-gray-600">
                            X轴颜色容差值 (Color Tolerance X)
                          </Label>
                          <div className="flex items-center gap-3 mt-2">
                            <Slider
                              value={[params.colorToleranceX]}
                              onValueChange={([v]) =>
                                handleParamChange("colorToleranceX", v)
                              }
                              min={5}
                              max={50}
                              step={1}
                        ...
                        <div>
                          <Label className="text-xs font-medium text-gray-600">
                            X轴连续判定宽度 (Sustained Pixels X)
                          </Label>
                          <div className="flex items-center gap-3 mt-2">
                            <Slider
                              value={[params.sustainedPixelsX]}
                              onValueChange={([v]) =>
                                handleParamChange("sustainedPixelsX", v)
                              }
                              min={5}
                              max={100}
                              step={5}
                        ...
                        <div className="border-t pt-4 mt-4">
                          <Label className="text-xs font-semibold text-orange-600 mb-3 block">
                            多行图标检测 (Multi-row Icons)
                          </Label>
                          <div className="space-y-4">
                            <div>
                              <Label className="text-xs font-medium text-gray-600">
                                第一行图标线偏移 (Icon Line Offset)
                              </Label>
                              <div className="flex items-center gap-3 mt-2">
                                <Slider
                                  value={[params.iconLineOffset]}
                                  onValueChange={([v]) =>
                                    handleParamChange("iconLineOffset", v)
                                  }
                                  min={50}
                                  max={300}
                                  step={1}
                              ...
                              <p className="text-xs text-gray-500 mt-1">
                                第一行icon检测线相对于panel顶部的偏移
                              </p>
                            </div>

                            <div>
                              <Label className="text-xs font-medium text-gray-600">
                                多行图标线间距 (Icon Line Gap)
                              </Label>
                              <div className="flex items-center gap-3 mt-2">
                                <Slider
                                  value={[params.iconLineGap]}
                                  onValueChange={([v]) =>
                                    handleParamChange("iconLineGap", v)
                                  }
                                  min={100}
                                  max={300}
                                  step={1}
                            ...
                            <div>
                              <Label className="text-xs font-medium text-gray-600">
                                每行最小图标数量 (Min Icons Per Line)
                              </Label>
                              <div className="flex items-center gap-3 mt-2">
                                <Slider
                                  value={[params.minIconsPerLine]}
                                  onValueChange={([v]) =>
                                    handleParamChange("minIconsPerLine", v)
                                  }
                                  min={1}
                                  max={10}
                                  step={1}
                ...
                <div className="border-l-4 border-indigo-500 pl-4 bg-indigo-50 p-3 rounded">
                  <Label className="text-sm font-semibold text-indigo-600 mb-3 block">
                    Bounds Detection
                  </Label>
                  <p className="text-xs text-indigo-700 mb-3">
                    Use sliding window color variance to detect row and column
                  ...
                  <div className="space-y-4">
                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        检测窗口高度 (Window Height)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.boundsWindowHeight]}
                          onValueChange={([v]) =>
                            handleParamChange("boundsWindowHeight", v)
                          }
                          min={1}
                          max={20}
                          step={1}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            handleParamChange("boundsWindowWidth", v)
                          }
                          min={1}
                          max={20}
                          step={1}
                    ...
                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        行检测方差阈值 (Row Variance)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.boundsVarianceThresholdRow]}
                          onValueChange={([v]) =>
                            handleParamChange("boundsVarianceThresholdRow", v)
                          }
                          min={10}
                          max={200}
                          step={5}
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        判断是否为行区域的最小方差值（阈值越高越严格）
                      </p>
                    </div>

                    <div>
                      <Label className="text-xs font-medium text-gray-600">
                        列检测方差阈值 (Col Variance)
                      </Label>
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          value={[params.boundsVarianceThresholdCol]}
                          onValueChange={([v]) =>
                            handleParamChange("boundsVarianceThresholdCol", v)
                          }
                          min={10}
                          max={200}
                          step={5}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            handleParamChange("boundsStepSize", v)
                          }
                          min={1}
                          max={10}
                          step={1}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            handleParamChange("boundsMinRowHeight", v)
                          }
                          min={5}
                          max={100}
                          step={5}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <Slider
                          ...
                          onValueChange={([v]) =>
                            handleParamChange("boundsMinColWidth", v)
                          }
                          min={5}
                          max={100}
                          step={5}
                    ...
                    <div className="pt-4 border-t border-indigo-200">
                      <Label className="text-xs font-semibold text-indigo-700 mb-3 block">
                        图标形状设置
                      </Label>
                        ...
                        <>
                          <div className="p-3 bg-indigo-50 rounded mb-3">
                            <p className="text-xs text-indigo-600">
                              <strong>工作原理：</strong>
                              基于原始边界计算方差和过滤，只在绘制和导出时应用1:1收束，确保输出正方形图标。
                            </p>
                          </div>
                          ...
                          <div className="p-3 bg-purple-50 rounded">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-sm font-bold text-purple-700">
                                {params.emptyIconVarianceThreshold}
                              </span>
                            </div>
                            <Slider
                ...
                <div className="pt-4 border-t">
                  ...
                  <div className="flex gap-2 flex-wrap mb-3">
                    ...
                    <Button
                      ...
                      className="flex-1 min-w-30 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300"
                    >
                      🔄 重置为新默认值
                    </Button>
                    <Button
                  ...
                  <div className="flex gap-2 flex-wrap mb-3">
                    <Button
                      ...
                      className="flex-1 min-w-30 bg-green-600 hover:bg-green-700 text-white"
                    >
                      ✨ 应用优化参数
                    </Button>
                  </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded text-xs space-y-1 max-h-40 overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between">
                      <span>
                        ...
                        <span className="font-mono text-blue-600 dark:text-blue-400">
                          {params.forceSquareOffsetX}
                        </span>
                      </span>
                      {params.forceSquareOffsetX !==
                        DEFAULT_PARAMS.forceSquareOffsetX && (
                    ...
                    <div className="flex items-center justify-between">
                      <span>
                        Y轴偏移:{" "}
                        <span className="font-mono text-blue-600 dark:text-blue-400">
                          {params.forceSquareOffsetY}
                        </span>
                      </span>
                      {params.forceSquareOffsetY !==
                        DEFAULT_PARAMS.forceSquareOffsetY && (
                        <span className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded">
                          已修改
                        </span>
                      )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {params.boundsVarianceThresholdRow !==
                        DEFAULT_PARAMS.boundsVarianceThresholdRow && (
                        <span className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded">
                          已修改
                        </span>
                      )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {params.boundsVarianceThresholdCol !==
                        DEFAULT_PARAMS.boundsVarianceThresholdCol && (
                        <span className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded">
                          已修改
                        </span>
                      )}
                    ...
                    <div className="flex items-center justify-between">
                      <span>
                        最小行高:{" "}
                        <span className="font-mono text-green-600 dark:text-green-400">
                          {params.boundsMinRowHeight}
                        </span>
                      </span>
                      {params.boundsMinRowHeight !==
                        DEFAULT_PARAMS.boundsMinRowHeight && (
                        <span className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded">
                          已修改
                        </span>
                      )}
                    ...
                    <div className="flex items-center justify-between">
                      <span>
                        最小列宽:{" "}
                        <span className="font-mono text-green-600 dark:text-green-400">
                          {params.boundsMinColWidth}
                        </span>
                      </span>
                      {params.boundsMinColWidth !==
                        DEFAULT_PARAMS.boundsMinColWidth && (
                        <span className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded">
                          已修改
                        </span>
                      )}
      ...
      {imageUrl &&
        ...
        (() => {
          const scaleRatioX =
            (canvasRef.current.clientWidth / canvasRef.current.width) * 100;
          const scaleRatioY =
            (canvasRef.current.clientHeight / canvasRef.current.height) * 100;
          const isScaled =
            Math.abs(scaleRatioX - 100) > 1 || Math.abs(scaleRatioY - 100) > 1;

          return (
            <Card className={`mt-6 ${isScaled ? "bg-red-50" : "bg-green-50"}`}>
              <CardHeader>
                <CardTitle
                  className={isScaled ? "text-red-800" : "text-green-800"}
                >
                  {isScaled
                    ? "⚠️ 警告：Canvas 被缩放了！"
                    : "✅ Canvas 缩放正常"}
              ...
              <CardContent
                className={`text-sm ${isScaled ? "text-red-900" : "text-green-900"}`}
              >
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p>
                      {canvasRef.current.clientWidth} x{" "}
                      {canvasRef.current.clientHeight} 像素
                    </p>
                  </div>
                  <div>
                    <strong>缩放比例：</strong>
                ...
                {isScaled && (
                  <div className="mt-4 p-3 bg-red-100 rounded border border-red-300">
                    ...
                    <p className="mt-2 text-red-900">
                      Canvas 被缩放了 {Math.abs(scaleRatioX - 100).toFixed(1)}
                      %，这会导致裁切坐标不准确！
                    </p>
                    <p className="mt-2 text-red-900">
                      <strong>可能原因：</strong>
          ...
          <CardContent>
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
              <p className="text-sm text-blue-900">
                <strong>💡 坐标显示说明：</strong>
                各框左上角会显示坐标值（格式：x,y）。裁切后的图片会自动在左下角添加红色坐标信息，便于对照验证。
              </p>
            </div>
            <div className="mt-4 p-3 bg-purple-50 rounded border border-purple-200">
              <p className="text-sm text-purple-900">
                <strong>🔍 滑动窗口检测（主要方法）：</strong>
                使用滑动窗口平均算法自动检测多行多列图标布局，无需手动调整间距。红色横向矩形窗口检测行，蓝色竖向矩形窗口检测列，窗口中心点作为图标起始坐标。
              </p>
            </div>
          </CardContent>
        </Card>
